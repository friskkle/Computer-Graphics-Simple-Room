...
vec4 pointLight(){
	...
	float specAmount = pow(max(dot(viewDirection, reflectionDirection), 0.2f), 8);
	...
	return (texture(tex0, texCoord) * lightColor * (diffuse * inten * 4 * (1.0f - shadow) + ambient + specular * (1.0f - shadow)));
}
...
vec4 direcLight()
{
	...
	return (texture(tex0, texCoord) * (diffuse * (1.0f - shadow) + ambient + specular * (1.0f - shadow)*0.7));
}
